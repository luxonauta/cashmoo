...
async function ensureUserProfile(): Promise<void> {
  const row = await queryGet<{ c: number }>("SELECT COUNT(1) as c FROM user_profile");
  if (!row || row.c === 0) {
    await queryRun("INSERT INTO user_profile(id,name) VALUES(1,'User')", []);
  }
}

async function findOpenInvoice(cardId: number, year: number, month: number): Promise<{ id: number } | undefined> {
...
async function createInvoice(cardId: number, year: number, month: number): Promise<void> {
  await queryRun(
...
async function refreshInvoiceTotal(cardId: number, year: number, month: number): Promise<void> {
...
export async function runScheduler(): Promise<void> {
  ...
  const cards = await queryAll<{ id: number; closingDay: number; dueDay: number }>(
  ...
  for (const c of cards) {
    const exists = await findOpenInvoice(c.id, periodStart.getFullYear(), periodStart.getMonth() + 1);
    if (!exists) {
      await createInvoice(c.id, periodStart.getFullYear(), periodStart.getMonth() + 1);
    }
    if (sameOrBefore(periodStart, periodEnd)) {
      await refreshInvoiceTotal(c.id, periodStart.getFullYear(), periodStart.getMonth() + 1);
    }
  }
}
...
export function formatRange(startISO: string, endISO: string): { start: string; end: string } {
