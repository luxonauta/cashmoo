...
export default function ExpensesView(): JSX.Element {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>): void {
    ...
    apiCreateExpense(payload).then((row) => setRows([row, ...rows]));
  }
    ...
    <section>
      ...
      <form onSubmit={onSubmit}>
        ...
        <input
          ...
          type="number"
        />
        <label htmlFor="eauto">Auto debit</label>
        <input
          id="eauto"
          checked={autoDebit}
          ...
          type="checkbox"
        />
        <label htmlFor="ecard">Card expense</label>
        <input
          id="ecard"
          checked={isCard}
          ...
          type="checkbox"
        />
        <label htmlFor="ecardid">Card</label>
        <select
          id="ecardid"
          value={cardId}
        ...
        <label htmlFor="efirst">First due date</label>
        <input
          id="efirst"
          value={firstDueDate}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td>{r.recurrence}</td>
              <td>{r.nextDate}</td>
              <td>{r.isCard ? "yes" : "no"}</td>
              <td>{r.paidAt ? r.paidAt : ""}</td>
              <td>
                <button onClick={() => onPay(r.id)} disabled={!!r.paidAt}>
                  Mark paid
                </button>
              </td>
            </tr>
